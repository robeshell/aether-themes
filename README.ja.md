...
**カスタマイズ可能なテンプレート型個人サイト向けの AI-first テーマキットです。**

Aether は、切り替え可能な CSS テーマレイヤーと、AI によるサイト構築・保守フローを組み合わせています。AI エージェントが編集可能なサイトテンプレートを作成・更新できるようにしながら、特定の人のコンテンツ、ルート、フレームワークには依存しません。テーマレイヤーは [W.Site](https://robeshell.github.io/) から抽出しました。
...
Aether は AI-first のテンプレート型テーマキットであり、完成済みのブログテーマ、サイトジェネレーター、CMS ではありません。次の 2 つのレイヤーで構成されています。

1. **テーマレイヤー** — フレームワーク非依存の CSS、任意のバニラ JS スニペット、デザイントークン、セマンティックなマークアップ契約。
2. **AI ビルダーレイヤー** — 要件の質問、編集可能なテンプレートの作成・更新、テーマ設定、結果の検証を行うプロンプトと Skill。

ルート、コンテンツモデル、管理画面、すべてのブログ基盤向けのテンプレートは提供しません。生成されたサイトは通常のプロジェクトとして動作し、実行時に AI サービスを必要としません。

Aether は、AI エージェントまたは開発者がテンプレートを編集できるサイトを対象としています。テンプレートにアクセスできないホスト型ブログを自動で再スタイルする汎用アダプターではありません。
...
| 環境 | 統合レベル |
| --- | --- |
| Astro | 正式なスターターとテンプレートのワークフローに対応 |
...
- **AI-first の構築と保守**のため、リポジトリから発見できる Skill と、コピーして使えるプロンプトを提供します。
- **npm のランタイム依存なし。** Persona のタイトル分割と共有 SVG パスは任意のバニラ JS スニペットとして提供します。ゲームや映像作品の著作権画像、スクレイピング素材は同梱しません。
...
AI エージェントの役割は、サイトの編集可能なテンプレートを作成・更新し、Aether のセマンティックフックを使うようにすることです。Aether は任意のマークアップを自動推測したり、テンプレートを編集できないホスト型ブログを書き換えたりはしません。
...
Aether は固定ルートのページを生成するのではなく、セマンティックフックにスタイルを適用します。必須フック、リッチコンテンツフック、任意のランタイムスニペットは [`THEME_CONTRACT.md`](./THEME_CONTRACT.md) を参照してください。
...
Persona の文字単位タイトル、区切り線の SVG パス、リスト矢印のパスはバニラ ESM に置いてあり、Astro コンポーネントなしでも import できます。
...
`html[data-theme]` が変わるたびに `applyThemeEnhancements` を呼んでください。テーマピッカーの UI と永続化はサイト側のままです。
